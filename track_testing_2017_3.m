...
%         ,0.15,'NumCascadeStages',11,'FeatureType','LBP');
% 
%     trainCascadeObjectDetector(str2,pos_2017,file2,'FalseAlarmRate'...
%         ,0.15,'NumCascadeStages',11,'FeatureType','Haar');    
    ...
    %th = (15/(60*expt.tracking(2).pixelsize))*expt.tracking(2).frameinterval;
    %th = 150;
    th = 75;
    %th = 50;
    th2 = 75;
...
%                 end
                ...
                pts_shift = pts_shift + 0.5*pts_diff2;
...
%     for i = 1:numel(bbox1(:,1))
%         dist = sqrt(((bbox1(i,1) + round(0.5*bbox1(i,3))) - (bbox2(:,1) + round(0.5*bbox2(:,3)))).^2 + ((bbox1(i,2) + round(0.5*bbox1(i,4))) - (bbox2(:,2) + round(0.5*bbox2(:,4)))).^2);
%         f = find(dist < 0.5*bbox1(i,3), 1);
%         if ~isempty(f)
%         	goods = [goods i];
%         end
%     end
%     
%     cent = [bbox1(goods,1) + round(0.5*bbox1(goods,3)) bbox1(goods,2) + round(0.5*bbox1(goods,4))];
