...
%Should probably insert some error checking here
    ...
    im2(Y:Y+p_size(1)-1,X:X+p_size(2)-1) = im_particle1;    
    ...
    mean_corners = mean([im_corner1(:);im_corner2(:);im_corner3(:);im_corner4(:)]);
    ...
    rand_noise = uint16(round(r2*rand(size(im_particle1)) - r2/2))    ;
